...
#Main program
def main(scene,common=[1024,512,['lit'],0,1,'cam_rot.txt','R','No','No','No'],specific=[180,120]):
	...
	for mode in mode_list:
		...
		for loc in loc_list:
			...
			image_path = "cylindric/{}/".format(mode)
			...
			if data == "Yes" and mode=='depth':
				...
				np.save("cylindric/depth_data/{}.npy".format(image_name),depth)
				np.save("cylindric/depth_data/{}-pcloud-{}-v{}-h{}-{}".format(scene,rot1[0]+rot2,vertical,horizontal,loc),point_cloud)
			if show == 'Yes':
				cv2.imshow('{}'.format(image_name),final)
				cv2.waitKey(0)
				cv2.destroyAllWindows()
			cv2.imwrite('{}.png'.format(image_path+image_name),final)
			fichero.main(image_path+image_name,'cylindric',common,specific,R_view,mode,loc) 
			if semantica=='Yes' and mode == "object_mask":
				result_path = "cylindric/binary_masks/"
