...
def get_images(mode,Zr,mirror,cam,R,vignette,threshold,code):
	...
	for index in range(len(rotation)//3):
		...
		time.sleep(0.01)	
		client.request('vget /camera/0/{} Pack/no-central/{}/{}{}.png'.format(mode,mirror,mode,index))
		...
		for mode in mode_list:
			...
			#Pixel mapping
				for r in np.arange(1,final_w/2,1):
					...
					get_images(mode,Z_r, mirror, camera_loc, R_world, vignette, threshold, code)
