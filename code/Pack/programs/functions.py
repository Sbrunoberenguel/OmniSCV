...
def load_img(mode,loc):
	...
	return [imagenx_pos,imagenx_neg,imageny_pos,imageny_neg,imagenz_pos,imagenz_neg]
...
def depth_image(depth,path,pos,index,vignette):
	#depth_img = np.zeros((1024,1024,3), np.uint8)
	dc_img = np.load(io.BytesIO(depth))
	if vignette:
