...
def get_images(mode,vignette,threshold,code,back_5):
	...
	for index in range(len(rotation)//3):
		...
		time.sleep(0.01)	
		client.request('vget /camera/0/{} {}/Pack/no-central/ncp/{}{}.png'.format(mode,back_5,mode,index))
		if mode == 'lit' and vignette:
			...
			d_img = np.load(io.BytesIO(d))
			f.depth_image2(d_img,"no-central/ncp/depth",index,threshold,code,vignette)
	rot_file.close()

#Main program
def main(vignette,scene,common=[1024,512,['lit'],10.0,1,'cam_rot.txt','R','No','No','No','grey'],specific=[100]):
		...
		for mode in mode_list:
			...
			for pos in range(len(location)-1):
				...
				if data == 'Yes' and mode == 'depth':
					...
					depth = np.zeros([final_h,final_w],np.float32)
				#Camera parameters
				...
				for i in range(final_w):
					...
					get_images(mode,vignette,threshold,code,back_5)
